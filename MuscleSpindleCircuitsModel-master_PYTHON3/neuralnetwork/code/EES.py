...
class EES(object):
	...
	def _load_rec_data(self):
		...
		if rank==0:
			...
			else: allPercIf_GM= old_div(recI_MG,max(recI_MG))
			if max(recII_MG) == 0: allPercIIf_GM = recII_MG
			else: allPercIIf_GM = old_div(recII_MG,max(recII_MG))
			if max(recMn_MG) == 0: allPercMn_GM  = recMn_MG
			else: allPercMn_GM =  old_div(recMn_MG,max(recMn_MG))
			if max(recI_TA) == 0: allPercIf_TA = recI_TA
			else: allPercIf_TA = old_div(recI_TA,max(recI_TA))
			if max(recII_TA) == 0: allPercIIf_TA = recII_TA
			else: allPercIIf_TA = old_div(recII_TA,max(recII_TA))
			if max(recMn_TA) == 0: allPercMn_TA = recMn_TA
			else: allPercMn_TA = old_div(recMn_TA,max(recMn_TA))

			self._minCur = 0 #uA
			self._maxCur = 600 #uA

			nVal = recI_MG.size
			allPercIf= old_div((allPercIf_GM+allPercIf_TA),2)
			allPercIIf= old_div((allPercIIf_GM+allPercIIf_TA),2)
			allPercMn = old_div((allPercMn_GM+allPercMn_TA),2)
