...
def main():
	...
	for n in range(args.nSim):
		...
		count+=1
		if old_div(count,args.nSim)-percLastPrint>=printPeriod:
			percLastPrint=old_div(count,args.nSim)
			print(str(round(old_div(count,args.nSim)*100))+"% of simulations performed...")
...
def plot_rec_curve(nSimulations,figName,outFileName,showPlot=False):
	...
	sem = old_div(np.std(delay,axis=0),np.sqrt(len(delay)-1))
	...
	sem = old_div(np.std(recCruve,axis=0),np.sqrt(len(recCruve)-1))
